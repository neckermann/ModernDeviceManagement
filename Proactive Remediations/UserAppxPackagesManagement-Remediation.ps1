<#
.SYNOPSIS
    Remediation of the AppxPackages to be removed
.DESCRIPTION
    Same
.NOTES
    Filename: UserAppxPackagesManagement-Remediation.ps1
    Version: 1.0.0
    Author: Nick Eckermann
    Email: nickeckermann@outlook.com
    1.0.0 Nick Eckermann
.LINK
    https://github.com/neckermann/ModernDeviceManagement/tree/main/Proactive%20Remediations\UserAppxPackageManagement-Remedation.ps1
#>
#Logging
# Logging Path
...
# Loop through installed pacakges to see if there are any to remove and remove them
foreach($AppxPackageInstalled in $AppxPackagesInstalled){
    if ($AppxPackageInstalled.Name -in $AppxPackagesToRemove){
        Write-Log -Message "$($AppxPackageInstalled.Name) should not be installed" `
                  -Path $LoggingPath `
                  -Component AppxPackagesManagement-Remediation.ps1 `
                  -LoggingCleanupDays 30 `
                  -Type Warning
        Write-Log -Message "Removing $($AppxPackageInstalled.Name)" 
                  -Path $LoggingPath `
                  -Component AppxPackagesManagement-Remediation.ps1 `
                  -LoggingCleanupDays 30 `
                  -Type Informational
        Remove-AppxPackage -Package $AppxPackageInstalled
        if(-not (Get-AppxPackage -Name $AppxPackageInstalled.Name)){
            Write-Log -Message "$($AppxPackageInstalled.Name) was removed" 
                      -Path $LoggingPath `
                      -Component AppxPackagesManagement-Remediation.ps1 `
                      -$LoggingCleanupDays 30 `
                      -Type Informational
            $RemediationResults += "$($AppxPackageInstalled.Name) was removed"
        }else{
            Write-Log -Message "$($AppxPackageInstalled.Name) was not removed" 
                      -Path $LoggingPath `
                      -Component AppxPackagesManagement-Remediation.ps1 `
                      -$LoggingCleanupDays 30 `
                      -Type Error
            $RemediationResults += "$($AppxPackageInstalled.Name) was not removed"
            Write-Output ($RemediationResults -join ', ')
            Exit 1
        }
    }
}
